...
export default function CandlestickChart({ data, symbol, timeframe, setTimeframe }: ChartProps) {
  ...
  return (
    <div 
      ref={containerRef} 
      className={`flex flex-col bg-[#09090c] border border-[#13131a] rounded-xl overflow-hidden p-4 shadow-xl ${
        isFullscreen ? 'h-screen w-screen p-6' : 'h-[460px] w-full'
      }`}
    >
      {/* Chart Header Controls */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#13131a] pb-3 mb-3">
        {/* Left Side: Pair & Price details */}
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="font-bold text-base text-white tracking-wide">{symbol}</span>
            <span className="text-xs bg-[#13131a] text-[#8e8e9f] px-2 py-0.5 rounded uppercase">{timeframe}</span>
          </div>

          {selectedCandle && (
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-[#8e8e9f] number-mono">
              ...
              <span>H: <span className="text-white">${selectedCandle.high.toLocaleString(undefined, { minimumFractionDigits: symbol.startsWith('jpy') ? 6 : 2 })}</span></span>
              <span>L: <span className="text-white">${selectedCandle.low.toLocaleString(undefined, { minimumFractionDigits: symbol.startsWith('jpy') ? 6 : 2 })}</span></span>
        ...
        <div className="flex items-center gap-3">
          {/* Timeframes */}
          <div className="flex items-center bg-[#0d0d12] border border-[#13131a] rounded-lg p-0.5">
          ...
          <div className="h-4 w-[1px] bg-[#13131a]" />
          ...
          <button
            ...
            className={`p-1.5 rounded-lg border transition-all ${
              showIndicators 
                ...
                : 'bg-[#0d0d12] border-[#13131a] text-[#8e8e9f] hover:text-white'
            }`}
          ...
          <button
            className="p-1.5 rounded-lg bg-[#0d0d12] border border-[#13131a] text-[#8e8e9f] hover:text-white transition-all"
